...
# estimates <- rlasso_cox(mibase, 7:706, "OS", "Exitus", l = 0.3, lstd = F)
# estimates_f <- 
#   estimates |> mutate(HRs_unit = exp(fcoefs^2*10000)) |> 
#   filter(HRs_unit > 1.001) |> 
#   mutate(names = str_replace_all(vari, "_log2", ""))|> 
#   select(vari, fcoefs) |> 
#   rename(x = vari,
#          coefs = fcoefs)
# 
...
# mibase <-
#   mibase |> 
#   mutate(status = if_else(toupper(Exitus) %in% c("Y", "YES"), 1, 0), .before = OS)
# 
# x2 <- 
#   mibase |> 
#   mutate(status = if_else(toupper(Exitus) %in% c("Y", "YES"), 1, 0), .before = OS) |> 
...
#   select(-c(1:8))
#   
# tiempo <-  mibase[samp, "OS"] |> pull(OS)
# espacio <-  mibase[samp, "status"] |> pull(status)
#   
# y2 <- Surv(tiempo, 
#            espacio)
# 
# cindex(estimates_f, x2, y2)
